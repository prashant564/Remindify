...
//  Remindify
//
//  Created by PrashantDixit on 18/06/23.
//
...
class ReminderListViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        // adding the Add (+) button to add a new reminder
        navigationItem.rightBarButtonItem = UIBarButtonItem(
            barButtonSystemItem: .add, target: self, action: #selector(didPressAddButton(_:))
        )
        
        // adding the segmented buttons for filtering reminders based on date
    ...
    private func makeSwipeActions(for indexPath: IndexPath?) -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(
            style: .destructive, title: deleteActionTitle, handler: {
